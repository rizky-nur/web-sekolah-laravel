...
<html lang="en">
  ...
  <body>
    ...
    <h1 class="text-center pt-3">Info Sekolah</h1>
    <div class="container mt-3">
      <div class="row">
        <div class="col text-center">
          <h3 class="text-primary">Profile Sekolah</h3>
          <div class="border border-2 border-warning"></div>
          <div id="carouselExampleSlidesOnly" class="carousel slide mt-3" data-bs-ride="carousel">
            <div class="carousel-inner">
              @foreach ($profiles as $profile)
              <div class="carousel-item active">
                <img src="{{ asset('storage/' . $profile->image) }}"class="d-block w-100" alt="" srcset="">
              </div>
              @endforeach
            </div>
          </div>
        ...
        <div class="col">
          ...
          <h3 class="text-primary text-center">{{$pengumuman->title}}</h3>
          <div class="border border-2 border-warning"></div>
          <p class="pt-3">{!!$pengumuman->excrept!!}</p>
          <a href="/pengumuman/read/{{ $pengumuman->slug }}" class="btn btn-primary">Baca Selengkapnya</a>
          @endforeach
        </div>
      </div>
      <div class="shadow mt-5">
        @foreach ($sambutans as $sambutan)
        <h3 class="text-primary text-center">{{$sambutan->title}}</h3>
        <div class="border border-2 border-warning"></div>
        <div class="d-flex">
          <div class="row mt-3 mb-5">
            <div class="col text-center">
              <img src="{{ asset('storage/' . $sambutan->image) }}"class="width="257" height="314"" alt="" srcset="">
            </div>
            <div class="col">
              <p class="ps-3">{!!$sambutan->description!!}</p>
            </div>
          </div>
        </div>
        @endforeach
        ...
        <div class="col">
          ...
          <div id="carouselExampleSlidesOnly" class="carousel slide mt-3" data-bs-ride="carousel">
            <div class="carousel-inner">
              ...
              <div class="carousel-item active">
                <img src="{{ asset('storage/' . $berita->image) }}"class="d-block w-100" alt="" srcset="">
              </div>
              @endforeach
            </div>
          </div>
          @foreach ($beritas as $berita)
          <a href="/berita/read/{{ $berita->slug }}" class="text-decoration-none link-dark">
            <div class="d-flex mt-5 shadow">
            <img src="{{ asset('storage/' . $berita->image) }}" alt="" srcset="">
            <h6 class="ms-3">{{$berita->title}}</h6>
            </div>
          </a>
          @endforeach
        </div>
